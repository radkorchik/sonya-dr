    ...
    <div className="page-container">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <h1 className="font-display text-2xl font-bold text-ink-900">Сказки</h1>
        </div>
        <Link to="/tales/add" className="glass rounded-full p-2 text-pink-500" aria-label="Добавить сказку">
          <Plus size={20} />
        </Link>
      </div>
      ...
      {pageItems.length === 0 ? (
        ...
        <div className="space-y-3">
          {pageItems.map(tale => (
            <GlassCard key={tale.id} className="flex gap-2 items-center">
              ...
              <div className="flex items-center shrink-0">
                <Link to={`/tales/${tale.id}/edit`} className="p-2 text-ink-400" aria-label="Редактировать">
                  <Pencil size={16} />
                </Link>
                <button type="button" onClick={() => handleDelete(tale.id)} className="p-2 text-pink-400" aria-label="Удалить">
                  <Trash2 size={16} />
                </button>
                <FavoriteButton
      ...
      {totalPages > 1 && (
        <div className="flex justify-center gap-2 mt-4">
          <button type="button" disabled={page === 0} onClick={() => setPage(p => p - 1)} className="glass rounded-full px-4 py-2 text-sm disabled:opacity-40">←</button>
          <span className="text-sm text-ink-500 self-center">{page + 1} / {totalPages}</span>
          <button type="button" disabled={page >= totalPages - 1} onClick={() => setPage(p => p + 1)} className="glass rounded-full px-4 py-2 text-sm disabled:opacity-40">→</button>
        </div>
      )}
    </div>
  )
